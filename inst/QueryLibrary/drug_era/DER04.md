...
FROM
        (
        SELECT        SUM(date_part('day',r.drug_era_end_date - r.drug_era_start_date)) AS totExposure,
                        r.person_id
        FROM        @cdm.drug_era r
        WHERE
              r.drug_concept_id         = $1
        ...
        (
        SELECT        sum(date_part('day',p.observation_period_end_date-p.observation_period_start_date)) AS totalObs,
