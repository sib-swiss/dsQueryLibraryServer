...
FROM
        (
        SELECT        SUM((r.drug_era_end_date - r.drug_era_start_date)*interval '1 day')::numeric AS totExposure,
                        r.person_id
        FROM        @cdm.drug_era r
        WHERE
              r.drug_concept_id         = $1
        ...
        (
        SELECT        sum((p.observation_period_end_date-p.observation_period_start_date)*interval '1 day')::numeric AS totalObs,
