<!---
Group:vocabulary
Name:V1 Concept mapping
Author:Dmitry Kuznetsov
CDM Version: 5.3
-->

# V1: Statistics about concept mappings
...
## Query
```sql
 select tbl as table, clmn as column_name, tbl_concept_id, CONCEPT_ID as concept_id, concept_name, vocabulary_id, count(1)::integer as "count"  from         (select 'PERSON               ' as tbl,'GENDER_CONCEPT_ID             ' as clmn,gender_concept_id               as tbl_concept_id              ,CONCEPT_ID,concept_name,vocabulary_id from PERSON left join concept  on (gender_concept_id = CONCEPT_ID)
...
union all select 'VISIT_OCCURRENCE     ' as tbl,'ADMITTING_SOURCE_CONCEPT_ID   ' as clmn,admitting_source_concept_id     as tbl_concept_id    ,CONCEPT_ID,concept_name,vocabulary_id from VISIT_OCCURRENCE left join concept  on (admitting_source_concept_id = CONCEPT_ID)
union all select 'VISIT_OCCURRENCE     ' as tbl,'DISCHARGE_TO_CONCEPT_ID       ' as clmn,discharge_to_concept_id         as tbl_concept_id    ,CONCEPT_ID,concept_name,vocabulary_id from VISIT_OCCURRENCE left join concept  on (discharge_to_concept_id = CONCEPT_ID)
...
union all select 'VISIT_DETAIL         ' as tbl,'ADMITTING_SOURCE_CONCEPT_ID   ' as clmn,admitting_source_concept_id     as tbl_concept_id        ,CONCEPT_ID,concept_name,vocabulary_id from VISIT_DETAIL left join concept  on (admitting_source_concept_id = CONCEPT_ID)
union all select 'VISIT_DETAIL         ' as tbl,'DISCHARGE_TO_CONCEPT_ID       ' as clmn,discharge_to_concept_id         as tbl_concept_id        ,CONCEPT_ID,concept_name,vocabulary_id from VISIT_DETAIL left join concept  on (discharge_to_concept_id = CONCEPT_ID)
...
union all select 'ATTRIBUTE_DEFINITION ' as tbl,'ATTRIBUTE_TYPE_CONCEPT_ID     ' as clmn,attribute_type_concept_id       as tbl_concept_id,CONCEPT_ID,concept_name,vocabulary_id from ATTRIBUTE_DEFINITION left join concept  on (attribute_type_concept_id = CONCEPT_ID)) as a group by tbl,clmn,tbl_concept_id,CONCEPT_ID,concept_name,vocabulary_id
